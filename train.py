...
This module trains a DeepLab-style model, tracks losses and IoU metrics, and
saves the best checkpoint based on validation mean IoU.
"""
...
import config
from dataset import CopyPasteDataset, DesertDataset, get_train_transforms, get_val_test_transforms
...
def create_dataloaders(batch_size: int, device: torch.device) -> tuple[DataLoader, DataLoader]:
    ...
    if config.USE_COPY_PASTE:
        print(
            ...
            f"rare_classes={config.COPY_PASTE_RARE_CLASSES}  "
            f"prob={config.COPY_PASTE_PROB}"
        )
        train_dataset = CopyPasteDataset(
            train_dataset,
            rare_classes=config.COPY_PASTE_RARE_CLASSES,
            prob=config.COPY_PASTE_PROB,
        )
...
def run_validation(
    ...
    val_iou, _ = compute_mean_iou(confusion)
    return val_loss, val_iou



def train_model(batch_size: int) -> float:
    """Run full training loop and return best validation IoU.
    ...
    override_epochs = os.getenv("TRAIN_EPOCHS_OVERRIDE")
    if override_epochs:
        epochs = int(override_epochs)
    if device.type == "cpu" and config.CPU_QUICK_MODE:
        epochs = min(config.EPOCHS, config.CPU_QUICK_EPOCHS)
    scheduler_t_max = min(config.SCHEDULER_T_MAX, epochs)
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            ...
            f"No improve: {no_improve_count}/{config.EARLY_STOPPING_PATIENCE}"
        )
        ...
        if no_improve_count >= config.EARLY_STOPPING_PATIENCE:
            print(
                ...
                f"Best epoch: {best_epoch} | Best val mIoU: {best_val_iou:.4f}"
            )
            print(f"Best checkpoint already saved at: {config.BEST_MODEL_PATH}")
            break

    print(f"Training complete. Best validation mIoU: {best_val_iou:.4f}")
    return best_val_iou



def main() -> None:
    """Entry point for model training with OOM fallback batch sizing."""
    set_seed(config.SEED)

    try:
        train_model(batch_size=config.BATCH_SIZE)
        ...
        if "out of memory" in str(error).lower() and config.BATCH_SIZE != config.FALLBACK_BATCH_SIZE:
            ...
            train_model(batch_size=config.FALLBACK_BATCH_SIZE)
        else:
            raise
